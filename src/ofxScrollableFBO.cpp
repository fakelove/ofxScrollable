...
ofxScrollableFBO::ofxScrollableFBO(){
    p=false;
    pOrigin=0;
    desOrigin=0;
...
void ofxScrollableFBO::load(string path, float w, float h, float f){
    width = w;
    height = h;
...
void ofxScrollableFBO::load(ofPixels text, float w, float h, float f){
    width = w;
    height = h;
...
bool ofxScrollableFBO::pressed(ofPoint pos, int ID){
    p=true;
    pID=ID;
    pOrigin=pos.y;
...
bool ofxScrollableFBO::dragged(ofPoint pos, int ID){
    if(p && pID==ID){
...
bool ofxScrollableFBO::released(ofPoint pos, int ID){
    if(p && pID==ID){
        destination = desOrigin + (pos.y - pOrigin);
        if(texHeight<height){
            destination = 0;
...
float ofxScrollableFBO::getWidth(){
    return width;
}
float ofxScrollableFBO::getHeight(){
    return height;
}

void ofxScrollableFBO::reset(){
    position=0;
    destination=0;
    velocity=0;
}